...
let rec normalise_type mode t =
  ...
  | Type.ListT t -> Type.ListT (normalise_type mode t)
  | Type.VarT r -> begin
      match !r with
      | None ->  begin
          match mode with
...
let rec normalise_val mode v=
  ...
  | FunVal (f, ft, p, pt, e, gen) ->
     let newgen = match gen with
       | None -> None
       | Some (ws, ls, gprop) ->
          let newws = List.map (fun (w,t,m) -> w,normalise_type mode t,m) ws in
  ...
  | AbsCon (i, t, s, m) -> AbsCon (i, normalise_type mode t, s, m)
  ...
  | ListVal (ls,t) -> (** TODO: check if identity function is correct here **)
     let newls = SymList.map (fun v -> normalise_val mode v) (fun x -> x) ls in
     ListVal (newls, normalise_type mode t)
...
and normalise_exp mode e =
  ...
  | CondExp (e1, e2, e3, p) ->
     CondExp (normalise_exp mode e1, normalise_exp mode e2, normalise_exp mode e3, p)
  | NewRefExp (l, lt, e1, e2, p) ->
     NewRefExp (l, normalise_type mode lt, normalise_exp mode e1, normalise_exp mode e2, p)
  | DerefExp  _ -> e
  | AssignExp (l, e, p) -> AssignExp (l, normalise_exp mode e, p)
  | LetExp (i, it, e1, e2, p) ->
     LetExp (i, normalise_type mode it, normalise_exp mode e1, normalise_exp mode e2, p)
  | LetTuple (is_ts, e1, e2, p) ->
     LetTuple
       (List.map (fun (i, t) -> (i, normalise_type mode t))
          is_ts, normalise_exp mode e1, normalise_exp mode e2, p)
  ...
  | TupleUpdExp  (e1, i, j, e2, p) ->
     TupleUpdExp  (normalise_exp mode e1, i, j, normalise_exp mode e2, p)
  | MatchExp (t,e1,e2,i1,i2,e3,p) ->
     MatchExp (normalise_type mode t,
               normalise_exp mode e1,normalise_exp mode e2,i1,i2,normalise_exp mode e3,p)
...
let rec tref_in_type r = function
  ...
  | Type.IntT -> false
  | Type.FunT (ts, t2) ->
     (List.fold_left (fun b ty -> b || (tref_in_type r ty)) false ts) || (tref_in_type r t2)
  | Type.TupleT tlst -> List.exists (tref_in_type r) tlst
  | Type.ListT t -> tref_in_type r t
  | Type.VarT tr ->
     (if r == tr then true
      else match !tr with
           | None -> false
...
let rec unify p t1 t2 =
  ...
  | Type.ListT t1 , Type.ListT t2 -> unify p t1 t2
...
let op_type = function
  ...
  | ListCons ->
     ...
     Type.FunT ([ty; Type.ListT ty], Type.ListT ty)
  | PrintOp ->
     let ty = Type.gen_type () in          (* polymorphic print *)
     Type.FunT ([ty], Type.UnitT)
...
let rec type_of_val tenv v =
  ...
  | FunVal (f, ft, p, pt, e1, gen) ->
     ...
     begin
       ...
       | Some (idts , loc_exp_list , g_prop) ->
          let new_tenv =
            List.fold_right
              (fun (i,it,m) tenv -> extend i.idid it tenv)
              idts tenv
          in
          (List.iter
             (fun ((l,p),exp) ->
  ...
  | AbsFun (uid, t1, t2, s, gen) ->
     ...
     Type.FunT ([t1], t2)
  | ListVal (ls, t) -> SymList.iter (type_of_val tenv) (fun _ -> ()) ls; Type.ListT t
...
and type_of_exp tenv e =
  try
    ...
    | DerefExp (l, _) -> begin
        match lookup l.locid tenv with
        | None -> raise (Error.TypeE(Ast.get_lex_pos e, "unbound location: " ^ l.str))
        (* this is checked in ast.ml so it should not be raised here *)
        | Some ty -> ty
      end
    ...
    | LetTuple (is_ts, e1, e2, _) ->
       ...
       let new_tenv =
         List.fold_left (fun new_tenv (i, it) -> extend i.idid it new_tenv) tenv is_ts in
       type_of_exp new_tenv e2
    | SeqExp (e1, e2, _) ->
    ...
    | TupleProjExp (e1, i, j, p) ->
       if i >= j then
         raise (Error.TypeE(Ast.get_lex_pos e, "incorrect tuple projection: projecting element "
                                               ^ (string_of_int i) ^ " out of " ^ (string_of_int j)))
       else
         let tylst =
           (let rec loop j = if j = 0 then [] else (Type.gen_type ()) :: loop (j-1) in loop j) in
    ...
    | TupleUpdExp  (e1, i, j, e2, p) ->
       if i >= j then
         raise (Error.TypeE(Ast.get_lex_pos e, "incorrect tuple update: projecting element "
                                               ^ (string_of_int i) ^ " out of " ^ (string_of_int j)))
       else
         let tylst =
           (let rec loop j = if j = 0 then [] else (Type.gen_type ()) :: loop (j-1) in loop j) in
         ...
         ty
    | MatchExp (t,e1,e2,i1,i2,e3,p) ->
       unify (Ast.get_lex_pos e1) (Type.ListT t) (type_of_exp tenv e1);
       let tenv1 = extend i1.idid t tenv in
       let tenv2 = extend i2.idid (Type.ListT t) tenv1 in
       let t2 = (type_of_exp tenv2 e2) in
       unify (Ast.get_lex_pos e3) t2 (type_of_exp tenv2 e3);
       t2
...
and debug_get_tvars_val = function
  ...
  | AbsCon (i, it, s, m) -> (debug_get_tvars_type it)
